...
struct alignas(16) result_struct //16 bytes
{
	vec2h uv;
	uint32_t hit_light;
	float dist;
...
class vk_accelerator3 : public accelerator
{
	VkDevice dev;
	...
	public:
	vk_accelerator3(vec3i jd, vec3i sz, scene* s);

	~vk_accelerator3() //cleans up some of the garbage
	{
	...
	void intersect_n(int32_t id, int32_t index, ray_bundle &bundle)
	{
		...
		for(int32_t i = 0; i < job_size; i++)
		{
			...
			h_rec.hit_light = result.hit_light;
			...
			if(result.dist != -1.0f)
			{
				...
				h_rec.wi = bundle.r[i].d;
				h_rec.pos = result.dist * bundle.r[i].d + bundle.r[i].o;
				h_rec.dist = result.dist;
				vec2f uv;
				uv.x = result.uv.x;
				uv.y = result.uv.y;
