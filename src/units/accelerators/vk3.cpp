...
vk_accelerator3::vk_accelerator3(vec3i jd, vec3i sz, scene* s) : accelerator(s)
{
	job_dims = jd;
...
void vk_accelerator3::assemble_everything()
{
	...
	cmd_pool.create(dev,VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT,compute_index);
	as.create(dev,p_dev,cmd_pool.handle,queue,vertices.size(),(float*)vertices.data());
	//cmd_pool[0].destroy();
	...
	for(int32_t i = 0; i < SETS; i++)
		cmd_buff[i].create(dev,cmd_pool.handle);
