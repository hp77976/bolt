...
class partitioned_sampler : public sampler
{
	public:
	partitioned_sampler(int offset) : sampler(offset) {};

	~partitioned_sampler() {};

	partitioned_sampler* clone(uint32_t offset) const
	{
		return new partitioned_sampler(offset);
	};
	...
	{
		for(int i = 0; i < SAMPLER_DEPTH / 2; i++)
			f_array[i] = rng->get_float();

		for(int i = 0; i < SAMPLER_DEPTH / 2; i++)
			f_array[SAMPLER_DEPTH/2+i] = 1.0f - f_array[i];
